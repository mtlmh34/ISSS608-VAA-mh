...
description: |
  To visualizae financial data using R.
author:
  - name: Liang Minghao
...
## Select only top 40 companies
```{r}
company <- read_csv("../Data/s10/companySG.csv")
Top40 <- company %>% 
  slice_max(`marketcap`, n=40) %>% 
  select(symbol)

```
## Fetch stock prices within a period
```{r}
Stock40_daily <- Top40 %>%
...
# 1. Plot horizon Graph: GGplot
```{r}
# left join company to get full name
...
# 2. Interactive Line Chart: Plotly
